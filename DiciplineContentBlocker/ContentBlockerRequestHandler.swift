    ...
    func beginRequest(with context: NSExtensionContext) {
        ...
        if isBlocking {
            ...
            loadBlockingRules { rules in
                ...
                response.attachments = [NSItemProvider(item: rules as NSSecureCoding, typeIdentifier: UTType.json.identifier)]
            ...
            let emptyRules = "[]"
            let response = NSExtensionItem()
            response.attachments = [NSItemProvider(item: emptyRules as NSSecureCoding, typeIdentifier: UTType.json.identifier)]
            context.completeRequest(returningItems: [response], completionHandler: nil)
        }
    }
